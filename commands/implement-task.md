...
1. **Test-writer subagent** — writes a FAILING test pinning the task's acceptance
   criteria. It must NOT implement the feature; the test fails because the feature
   is absent (not a syntax error). Save under `tests/`.
